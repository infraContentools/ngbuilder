...
ModuleBuilder.prototype.watch = function watch() {
	...
	function buildDone(error) {
		if (error) {
			buildError(error, context.moduleName);
		} else {
	...
	this.buildSteps.forEach(function(step) {
		var stepPath = path.join(context.modulePath, step.path);
		logger.debug('ModuleBuilder: watching', stepPath);

		gulp.watch(stepPath).on('change', function(whatChanged) {
	...
	async.series(
		...
		function done(error) {
			if (error) {
				...
				logger.info('Rebuilt module ' + context.moduleName);
			}
...
function buildError(error, moduleName) {
	logger.info('>> Failed to build ' + moduleName);
	logger.error(error);
}